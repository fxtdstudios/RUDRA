...
| Video decode, encode | Phase 4 (section 17): steps 1 to 4 of 12 done: the probe, clock and input contract equal to `rudra/video.py` on 24 cases (`media/video_probe`, `platform/process`), the decoder's command and frames equal to the Python's (`media/video_decode`), and the video predictor with its shadow smoother within 2.7e-6 of eager PyTorch with the same cuts (`core/video_predict`, `engine/video_predictor`), and the mastering and 16-bit PNG spool with MaxCLL and MaxFALL as the Python computes them (`deliver/video_master`, `media/png16`) |
