...
# ── Reuse soft-knee compress from new package layout ──────────────────────
try:
    from radiance.nodes.hdr.encoder import _hdr_soft_compress, _compute_channel_stats
except ImportError:
    try:
        from nodes.hdr.encoder import _hdr_soft_compress, _compute_channel_stats
    except ImportError:
        # Standalone fallback — these are only used by the ONLINE cache-encoding
        # path (HDRLoRADataset._compress_frame); pair_dir training never calls
        # them, so simple implementations are sufficient to let the module import
        # outside the full ComfyUI 'radiance' package.
        def _hdr_soft_compress(x, ratio: float = 0.5):
            import torch
            return torch.log1p(x.clamp(min=0.0)) * float(ratio)

        def _compute_channel_stats(x):
            dims = tuple(d for d in range(x.ndim) if d != 1)
            return x.mean(dim=dims), x.std(dim=dims).clamp(min=1e-6)
...
try:
    ...
    try:
        ...
        # Standalone fallback via the project's own model registry (config/model_map).
        try:
            import importlib.util as _ilu
            _mm_path = os.path.join(os.path.dirname(__file__), "..", "config", "model_map.py")
            _spec = _ilu.spec_from_file_location("model_map", _mm_path)
            _mm = _ilu.module_from_spec(_spec); _spec.loader.exec_module(_mm)
            RADIANCE_MODEL_PRESETS = dict(_mm.MODEL_VAE_CONFIG)
        except Exception:
            RADIANCE_MODEL_PRESETS = {}

        def _resolve_model(name):
            key = (name or "").strip().lower()
            for k, v in RADIANCE_MODEL_PRESETS.items():
                if k in key or key in k:
                    return v
            return RADIANCE_MODEL_PRESETS.get(key)
...
class HDRLoRADataset(Dataset):
    ...
    def __init__(
        ...
        # A true video sample is an ordered list of consecutive frames from a
        # frozen manifest. Never fabricate motion by repeating a still frame.
        if self.n_frames > 1:
            if not manifest_path:
                raise ValueError(
                    "n_frames > 1 requires --manifest_path generated by "
                    "training/build_video_manifest.py"
                )
            self.exr_paths = self._load_manifest(manifest_path)
        else:
        ...
        if cache_dir:
            ...
            existing = sorted(Path(cache_dir).glob("*.npz"))
            if not self.exr_paths and existing:
                self._cache = existing
            elif self.exr_paths:
                self._cache = self._build_or_load_cache()
            else:
                raise FileNotFoundError(
                    f"No source frames and no .npz cache files found: {cache_dir}"
                )
        else:
        ...
        logger.info(
            ...
            len(self._cache) if self._cache is not None else len(self.exr_paths),
    ...
    def _load_manifest(self, manifest_path: str) -> List[Tuple[str, ...]]:
        clips: List[Tuple[str, ...]] = []
        with open(manifest_path, "r", encoding="utf-8") as handle:
            for line_no, line in enumerate(handle, start=1):
                if not line.strip():
                    continue
                record = json.loads(line)
                frames = record.get("hdr_frames")
                if not isinstance(frames, list) or len(frames) != self.n_frames:
                    raise ValueError(
                        f"{manifest_path}:{line_no}: expected exactly "
                        f"{self.n_frames} hdr_frames"
                    )
                missing = [p for p in frames if not os.path.isfile(p)]
                if missing:
                    raise FileNotFoundError(
                        f"{manifest_path}:{line_no}: missing frame {missing[0]}"
                    )
                clips.append(tuple(frames))
        if not clips:
            raise ValueError(f"No valid clips in manifest: {manifest_path}")
        return clips

    # ── Cache ─────────────────────────────────────────────────────────────────

    def _cache_key(self, path: Union[str, Sequence[str]]) -> str:
        source = "|".join(path) if not isinstance(path, str) else path
        tag = f"{source}|{self.target_hw}|{self.compression_ratio}|{self.per_channel_norm}"
    ...
    def _build_or_load_cache(self) -> List[Path]:
        ...
        for path in tqdm(self.exr_paths, desc="Building LoRA cache"):
            ...
            if not out_path.exists():
                ...
                if latent is None:
                    continue
                dr_raw = self._compute_dr_raw(path)
                np.savez_compressed(
                    str(out_path),
                    clean_latent = latent.float().cpu().numpy(),
                    dr_raw       = dr_raw,
                    meta         = json.dumps({
                        "source":           list(path) if not isinstance(path, str) else path,
    ...
    def _compute_dr_raw(self, path: Union[str, Sequence[str]]) -> np.ndarray:
        """Compute a real clip-level radiometric descriptor for gated LoRA.

        Frame descriptors are extracted in scene-linear space and averaged over
        time. This makes cached video latents usable by Stage 2 without the old
        all-zero conditioning proxy.
        """
        ...
        for frame_path in paths:
            image = load_exr_as_linear(frame_path)
            if image is None:
                raise RuntimeError(f"Could not load HDR frame for descriptor: {frame_path}")
            image = _centre_crop_resize(image, self.target_hw)
            frames.append(torch.from_numpy(image).permute(2, 0, 1))
        images = torch.stack(frames, dim=0).float()
        format_ids = torch.full((len(frames),), FORMAT_TO_ID["linear"], dtype=torch.long)
        descriptor = RUDRADescriptor(color_space="rec2020", normalize_input=True).eval()
        with torch.no_grad():
            dr_raw = descriptor(images, format_ids).mean(dim=0)
        return dr_raw.cpu().numpy().astype(np.float32)

    # ── Encode a single EXR path to a clean latent ───────────────────────────

    def _encode_path(self, path: Union[str, Sequence[str]]) -> Optional[torch.Tensor]:
        """Returns clean_latent tensor or None on failure."""
        paths = [path] if isinstance(path, str) else list(path)
        compressed_frames = []
        for frame_path in paths:
            img_linear = load_exr_as_linear(frame_path)
            if img_linear is None:
                raise RuntimeError(f"Could not load HDR frame: {frame_path}")
        ...
        if self.n_frames > 1:
            # True clip layout: (B, T, H, W, C). StandaloneVAE converts this
            # to the (B, C, T, H, W) layout required by video VAEs.
            pixel_batch = torch.from_numpy(np.stack(compressed_frames, axis=0)).unsqueeze(0)
        else:
            pixel_batch = torch.from_numpy(compressed_frames[0]).unsqueeze(0)
        ...
        try:
            ...
            raise RuntimeError(f"VAE encode failed for {path}: {e}") from e
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self._cache:
            ...
            dr_raw = (
                torch.from_numpy(data["dr_raw"].astype(np.float32))
                if "dr_raw" in data.files else None
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manifest_path", default=None,
                        help="JSONL clip manifest; required when n_frames > 1")
