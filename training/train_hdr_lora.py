...
class LoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return (base + self.scale * delta.to(base.dtype)).clone()
...
def export_lora_safetensors(
    ...
    for path, lora in lora_layers.items():
        ...
        classname = type(lora).__name__
        if classname == "RUDRALoRALinear":
            state[f"{key_base}.lora_down.weight"] = lora.lora_down.weight.data.cpu()
            state[f"{key_base}.lora_up.weight"]   = lora.lora_up.weight.data.cpu()
            if getattr(lora, "dr_gate", None) is not None:
                state[f"{key_base}.dr_gate.weight"] = lora.dr_gate.weight.data.cpu()
                if getattr(lora.dr_gate, "bias", None) is not None:
                    state[f"{key_base}.dr_gate.bias"] = lora.dr_gate.bias.data.cpu()
        else:
...
def load_lora_safetensors(
    ...
    Finds each LoRALinear or RUDRALoRALinear in the model and loads the matching A/B weights.
    ...
    Args:
        ...
        strength:  Multiplier on the LoRA scale (1.0 = full strength,
                   0.5 = half, etc.).  Applied by scaling lora_B/lora_up.
    ...
    def _apply(module: nn.Module, prefix: str = ""):
        for name, child in module.named_children():
            ...
            elif classname == "RUDRALoRALinear":
                key_base = "lora_unet_" + full_path.replace(".", "_")
                a_key = f"{key_base}.lora_down.weight"
                b_key = f"{key_base}.lora_up.weight"
                g_w_key = f"{key_base}.dr_gate.weight"
                g_b_key = f"{key_base}.dr_gate.bias"
                if a_key in state and b_key in state:
                    child.lora_down.weight.data.copy_(state[a_key])
                    child.lora_up.weight.data.copy_(state[b_key] * strength)
                if g_w_key in state and getattr(child, "dr_gate", None) is not None:
                    child.dr_gate.weight.data.copy_(state[g_w_key])
                if g_b_key in state and getattr(child, "dr_gate", None) is not None and getattr(child.dr_gate, "bias", None) is not None:
                    child.dr_gate.bias.data.copy_(state[g_b_key])
            else:
...
class LoRAEMA:
    CATEGORY = "FXTD STUDIOS/Radiance/◎ Pipeline"
    """EMA over only the LoRA parameters (supports standard and RUDRA-Lite)."""

    def __init__(self, lora_layers: Dict[str, nn.Module], decay: float = 0.999):
        self.decay  = decay
        self.shadow = {}
        for k, v in lora_layers.items():
            if hasattr(v, "lora_A"):
                self.shadow[f"{k}.A"] = v.lora_A.data.clone().detach()
                self.shadow[f"{k}.B"] = v.lora_B.data.clone().detach()
            else:
                self.shadow[f"{k}.down"] = v.lora_down.weight.data.clone().detach()
                self.shadow[f"{k}.up"] = v.lora_up.weight.data.clone().detach()
                self.shadow[f"{k}.gate.w"] = v.dr_gate.weight.data.clone().detach()
                self.shadow[f"{k}.gate.b"] = v.dr_gate.bias.data.clone().detach()

    @torch.no_grad()
    def update(self, lora_layers: Dict[str, nn.Module]):
        for k, v in lora_layers.items():
            if hasattr(v, "lora_A"):
                self.shadow[f"{k}.A"].mul_(self.decay).add_(v.lora_A.data, alpha=1.0 - self.decay)
                self.shadow[f"{k}.B"].mul_(self.decay).add_(v.lora_B.data, alpha=1.0 - self.decay)
            else:
                self.shadow[f"{k}.down"].mul_(self.decay).add_(v.lora_down.weight.data, alpha=1.0 - self.decay)
                self.shadow[f"{k}.up"].mul_(self.decay).add_(v.lora_up.weight.data, alpha=1.0 - self.decay)
                self.shadow[f"{k}.gate.w"].mul_(self.decay).add_(v.dr_gate.weight.data, alpha=1.0 - self.decay)
                self.shadow[f"{k}.gate.b"].mul_(self.decay).add_(v.dr_gate.bias.data, alpha=1.0 - self.decay)

    def apply_to(self, lora_layers: Dict[str, nn.Module]):
        for k, v in lora_layers.items():
            if hasattr(v, "lora_A"):
                v.lora_A.data.copy_(self.shadow[f"{k}.A"])
                v.lora_B.data.copy_(self.shadow[f"{k}.B"])
            else:
                v.lora_down.weight.data.copy_(self.shadow[f"{k}.down"])
                v.lora_up.weight.data.copy_(self.shadow[f"{k}.up"])
                v.dr_gate.weight.data.copy_(self.shadow[f"{k}.gate.w"])
                v.dr_gate.bias.data.copy_(self.shadow[f"{k}.gate.b"])

    def restore_from(self, lora_layers: Dict[str, nn.Module], backup: Dict):
        for k, v in lora_layers.items():
            if hasattr(v, "lora_A"):
                v.lora_A.data.copy_(backup[f"{k}.A"])
                v.lora_B.data.copy_(backup[f"{k}.B"])
            else:
                v.lora_down.weight.data.copy_(backup[f"{k}.down"])
                v.lora_up.weight.data.copy_(backup[f"{k}.up"])
                v.dr_gate.weight.data.copy_(backup[f"{k}.gate.w"])
                v.dr_gate.bias.data.copy_(backup[f"{k}.gate.b"])

    def get_backup(self, lora_layers: Dict[str, nn.Module]) -> Dict[str, torch.Tensor]:
        backup = {}
        for k, v in lora_layers.items():
            if hasattr(v, "lora_A"):
                backup[f"{k}.A"] = v.lora_A.data.clone()
                backup[f"{k}.B"] = v.lora_B.data.clone()
            else:
                backup[f"{k}.down"] = v.lora_down.weight.data.clone()
                backup[f"{k}.up"] = v.lora_up.weight.data.clone()
                backup[f"{k}.gate.w"] = v.dr_gate.weight.data.clone()
                backup[f"{k}.gate.b"] = v.dr_gate.bias.data.clone()
        return backup
...
def load_diffusion_model(
    ...
    try:
        ...
        def differentiable_apply_mod(tensor, m_mult, m_add=None, modulation_dims=None):
            if modulation_dims is None:
                ...
                for d in modulation_dims:
                    out[:, d[0]:d[1]] = out[:, d[0]:d[1]] * m_mult[:, d[2]]
                    if m_add is not None:
                        out[:, d[0]:d[1]] = out[:, d[0]:d[1]] + m_add[:, d[2]]
                return out

        def double_stream_block_forward(self, img, txt, vec, pe, attn_mask=None, modulation_dims_img=None, modulation_dims_txt=None, transformer_options={}):
            if self.modulation:
            ...
            img_modulated = flux_layers.apply_mod(img_modulated, (1 + img_mod1.scale), img_mod1.shift, modulation_dims_img)
            ...
            txt_modulated = flux_layers.apply_mod(txt_modulated, (1 + txt_mod1.scale), txt_mod1.shift, modulation_dims_txt)
            ...
            # Some ComfyUI versions of DoubleStreamBlock don't define
            # `flipped_img_txt`; default to the standard (non-flipped) order.
            if getattr(self, "flipped_img_txt", False):
                ...
                attn = flux_layers.attention(q, k, v,
            ...
            # calculate the img blocks out-of-place (img = img + ...)
            img = img + flux_layers.apply_mod(self.img_attn.proj(img_attn), img_mod1.gate, None, modulation_dims_img)
            del img_attn
            img = img + flux_layers.apply_mod(self.img_mlp(flux_layers.apply_mod(self.img_norm2(img), (1 + img_mod2.scale), img_mod2.shift, modulation_dims_img)), img_mod2.gate, None, modulation_dims_img)

            # calculate the txt blocks out-of-place (txt = txt + ...)
            txt = txt + flux_layers.apply_mod(self.txt_attn.proj(txt_attn), txt_mod1.gate, None, modulation_dims_txt)
            del txt_attn
            txt = txt + flux_layers.apply_mod(self.txt_mlp(flux_layers.apply_mod(self.txt_norm2(txt), (1 + txt_mod2.scale), txt_mod2.shift, modulation_dims_txt)), txt_mod2.gate, None, modulation_dims_txt)
        ...
        def single_stream_block_forward(self, x, vec, pe, attn_mask=None, modulation_dims=None, transformer_options={}):
            if self.modulation:
                mod, _ = self.modulation(vec)
            else:
                mod = vec

            qkv, mlp = torch.split(self.linear1(flux_layers.apply_mod(self.pre_norm(x), (1 + mod.scale), mod.shift, modulation_dims)), [3 * self.hidden_size, self.mlp_hidden_dim_first], dim=-1)
            ...
            # compute attention
            attn = flux_layers.attention(q, k, v, pe=pe, mask=attn_mask, transformer_options=transformer_options)
            del q, k, v
            ...
            # Out-of-place addition (x = x + ...)
            x = x + flux_layers.apply_mod(output, mod.gate, None, modulation_dims)
            if x.dtype == torch.float16:
                x = torch.nan_to_num(x, nan=0.0, posinf=65504, neginf=-65504)
            return x

        flux_layers.apply_mod = differentiable_apply_mod
        flux_layers.DoubleStreamBlock.forward = double_stream_block_forward
        flux_layers.SingleStreamBlock.forward = single_stream_block_forward
        logger.info("[Patch] Successfully applied autograd patches for Flux Double/Single blocks and apply_mod.")
    except Exception as e:
        logger.warning("[Patch] Failed to apply autograd patches: %s", e)
    ...
    try:
        ...
        object.__setattr__(diffusion_model, "model_patcher", model_patcher)
        object.__setattr__(diffusion_model, "comfy_model", model_patcher.model)
...
def train(
    ...
    while step < steps:
        ...
        # ComfyUI BaseModel subclass names: "Flux"/"Flux2", "WAN21", "LTXV"/"LTXAV", etc.
        _comfy_cls_hdr = type(getattr(diffusion_model, "comfy_model", None)).__name__ if hasattr(diffusion_model, "comfy_model") else ""
        _dm_cls_hdr = type(diffusion_model).__name__
        _CONTEXT_MODELS_HDR = {
        ...
        if hasattr(diffusion_model, "comfy_model"):
            try:
                ...
                try:
                    ...
                    logger.warning(f"[Train] Direct call failed: {e2}. Last resort.")
                    pred = diffusion_model(noisy, t_val, t_emb)
        else:
            try:
                # 1. Try positional call (works for Flux/Wan/LTX and most diffusers models)
                if _uses_context_hdr:
                    pred = diffusion_model(noisy, t_val, t_emb)
                else:
        ...
        if step % eval_every == 0:
            backup = ema.get_backup(lora_layers)
                ...
                try:
                    ...
                    if hasattr(diffusion_model, "comfy_model"):
                        ...
                        eval_pred = diffusion_model(
                            eval_noisy_latent,
                            timestep = eval_t_val,
                            encoder_hidden_states = eval_t_emb,
                        )
        ...
        if step % save_every == 0 or step == steps:
            ...
            # Save EMA weights
            ema_backup = ema.get_backup(lora_layers)
            ema.apply_to(lora_layers)
