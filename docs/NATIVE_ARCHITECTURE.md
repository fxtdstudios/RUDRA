...
```
 ...
 ├──────────────────────────────────────────────────────────────────┤
 │ video/       convert_video: predictor, pipeline, QC (no Qt)      │
 ├───────────────┬───────────────┬───────────────┬──────────────────┤
 ...
 └──────────────────────────────────────────────────────────────────┘
 cli/  headless `rudra-native`: core + infer + media + deliver + video, no Qt
```
...
`cli/` must link without Qt. (`video/` joined in Phase 4, step 7: the video
pipeline needs infer, media and deliver together, and the CLI runs it, so it
sits above the three and below `engine/`, with no Qt and no GPU.)
...
| 2 | Actions: all 33 `data-act` ids as `QAction`s with the Studio's shortcuts and menus; the native menubar on macOS, in-window elsewhere | `ui/index.html` `data-act`, `SHORTCUTS` in `ui/app.js` | a test extracts both from the page and compares them to the app's actions | 1 |  **done** 24 Sep: `engine/actions` holds the Studio's 33 actions (the 32 menu ids and `wipe`) with the page's menus, labels, key hints, check states, keys, wipe nudge, Keyboard sheet and `refreshMenu` enable rules, plus the native ones (open a package or a folder, quit, and the viewer's modes, wipe, guides, aspect masks and view peak in a View menu, on keys the page leaves free); `tools/emit_actions_golden.py` reads the page (`tests/golden/actions`) and `test_actions` holds the table to it. The app's window moved to `app/main_window.cpp` (a library, `rudra_app_ui`); it builds its menubar and shortcuts from the table and runs every action through `run(id)`. `rudra_app_tests` (Qt, offscreen, in CI): the menubar is the spec entry for entry, the page's labels, hints and checks, every page key reaches its action and no key is on two, a real key press runs its action, and the handlers do what the page's do (strength 0.1 a press rounded to hundredths, which the app had at 0.05). Actions whose work is a later step are off and say which |
| 3 | Session model (`engine/session`): the page's `state` in C++, with undo and redo as commands (5.6) where the page pushes undo | `ui/app.js` `state`, `params()`, `pushUndo` | the same scripted actions give byte-identical `params()` JSON (goldens from the page, headless) | 1.5 |  **done** 24 Sep: `engine/session` is the page's grading state (mode, strength, preserve, regions, peak, container, wipe, the B flip) with `snapshot`, `pushUndo` (60 deep, redo cleared), `undo`, `redo` and the handlers that change it: the mode buttons, the strength slider (undo on press), the peak slider (not undone), Preserve, the Region EV drag (0.01 EV a pixel, 0.002 with Shift, clamped to 4 EV, rounded to hundredths, a press that never moves leaves no undo) and double click, the resets, the containers, the wipe keys and the keydown map; `js_number` prints as JavaScript does. `tools/emit_session_golden.py` runs the whole page in headless Chromium (app.js unmodified but for one line exposing its closure) and drives it with real DOM events over 6 scripts; `test_session` replays all 251 states and `params()` is the same bytes, with the same undo and redo depths and the same wipe, bit for bit. The app runs every session action through it, enables Undo and Redo from its depths, and follows a wipe dragged on the plate. The native View peak submenu went: the view peak is the session's peak slider, as on the page (step 6 draws it) |
| 4 | Main window: media rail (drop zone, shot list, open by path), viewer toolbar (compare, layer, probe, guides, zoom, HDR badge), transport (prev, play, next, timecode, scrub), right rail (scopes; Reconstruct, Grade, Deliver tabs; frame measurements), pipe bar; the simple and full workspaces | `ui/index.html`, the Pro-direction boards | every panel present and wired; a side-by-side screenshot review against the boards | 2.5 |  **done** 24 Sep: `app/layout.cpp` builds the page as widgets, each named by the page's id (`app/widgets.cpp`: the segments, check rows, panel labels, scrub bar and the icon and transport buttons drawn as the page's SVGs), and `app/theme/studio.qss.in` styles them as `ui/theme.css` does. `tools/emit_layout_golden.py` renders the page headless at 1600 x 1000 in five states (full, simple, rails and scopes hidden, the Grade and Deliver tabs) and `rudra_app_tests` holds the window to it: a widget for every page id (nine drawn by the viewer or its overlays, named with their reasons), shown and hidden as the page shows them in every state, the page's words (labels, buttons, tool tips, placeholders, checks, fields, notes, slider ranges), the frame within 1 px of the page's, and the shell's controls (workspace, icon rail, tabs, a menu item bringing its tab forward) doing what `shell.js` does. The session gained the Window state (rails, scopes), the Compare source and the view layer. Found on the way, a Studio defect: `index.html` never closed the right rail's `.railscroll`, so the colour pipeline bar the page calls always visible rendered as a 26 px square in the top right corner and Master EXR scrolled away with the column; closed, the bar spans the foot of the window and the actions stay put, as theme.css intended |
| 5 | Scope widgets: waveform, histogram and vectorscope painted from `ScopeData` as `drawScopes` and `drawVector` draw them (zone colours, gridlines, the dashed 203 line, the clip band) | `ui/app.js` `drawScopes`, `drawVector` | a raster of the page's scope SVG and the widget's paint agree within 2 codes on 99 % of pixels | 1.5 |  **done** 24 Sep: the page draws both scopes as SVG, a display list, so the oracle is exact: `core/scope_draw` builds the elements `drawScopes` writes, with the same attributes as the same strings (`js_to_fixed` is JavaScript's `toFixed`, halves up on the exact decimal), and `test_scope_draw` holds them to the page's markup (`tools/emit_scopes_golden.py`, the page headless on five sets of scope data: plain, clipped with MaxCLL, a sparse histogram, MaxCLL above the scale and below it). `app/scope_widgets` paints that list as a browser shows an `<svg>` (viewBox, xMidYMid meet, SVG white space, the thin-stroke rule) and the vectorscope as the page frames its canvas. Rasters, against the page's own element screenshots with Plex served to it: the geometry and colour (labels hidden) within a mean of 1 code and 97 % of pixels within 8 codes, each label where the page puts it within a pixel, the vectorscope within 98.5 %. The plan's 99 % within 2 codes could not hold between two antialiasing rasterisers on one-pixel zig-zag bands and 4 px glyphs; the exact display-list equality is the stronger check of what is drawn, and the raster bounds catch a missing band or a label two pixels out (both tried). Found on the way: the view snaps the vectorscope to whole pixels as the browser does, and draws small text unhinted at its real size |
| 6 | Reconstruct and Grade panels: mode, strength, preserve, view peak, the three-band Region EV editor (the `#regions` drag behaviour), anchor and chroma carry, container and primaries | `ui/app.js` handlers | each control drives the composite, the view and `params()` as the page does (the step 3 goldens) | 2 |  **done** 24 Sep: the controls of step 4 plus `app/region_editor` (the page's `#regions`: a row a band with its swatch, range, value and unit; a press selects, a drag on the value scrubs it in the value's own coordinates, which keep a high-DPI mouse's fractions, a double click zeroes it) and the keys the page's keydown takes before its map (the wipe nudge, Escape, B held). The session golden now also records what the panels show after every gesture (the Region EV rows with their live and selected states, the region count, the strength and peak read-outs, Preserve and its hint, the mode that is on), and `rudra_app_tests` makes all 251 of the page's gestures on the window's own widgets (button clicks, slider drags, pointer drags and double clicks on the EV values, menu items, keys with Shift) and reaches the page's `params()`, undo and redo depths, wipe and panel words at every step. `core/js_format` gained the page's `fmt` (en-US grouping), `signed` (U+2212) and `nitsLabel`. Two fixes on the way: the page maps "z" and "Z" alike, so the letter shortcuts work with Shift too; and the golden's double click is now a browser's (two presses and releases, then dblclick), which also selects the row |
| 7 | Probe: the floating box and the rail panel at the cursor (`probeAt`, `showProbePanel`): nits, stops, baseline and model values, masks, the SDR codes | `ui/app.js` | text identical to the page's for the same pixel | 0.5 |  **done** 24 Sep: `core/readouts` writes the probe panel and the floating box as `showProbePanel` and `showProbe` do (`tests/golden/readouts`: the page's writers run headless on seven pixels, from none to a clipped 12 346-nit specular), and the app reads the pixel under the pointer from the full-resolution composite the measurement keeps (`FrameMeasure::probe_at`, the viewer's `on_hover` in frame pixels); the box is a window of its own so it can sit over the viewer's swapchain, placed as the page places it. Probe on, or Alt held: the page's readout promised Alt and never did it, so the page's `pointermove` now honours Alt too |
| 8 | Frame measurements panel (`showMetrics`), the clip bar, the pipe bar's warning | `ui/app.js` `showMetrics`, `paintClipBar`, `updatePipe` | text identical to the page's for the same frame | 0.5 |  **done** 24 Sep: `engine/measure` is the page's `computeStats` for the app: the chain Phase 2 held to the browser (composite, corrected baseline, exact reductions, the sample, scopes, vectorscope, mask and clip coverage) run on a worker 90 ms after the grade settles, a stale result dropped; `Viewer.MeasureFrameIsThePagesComputeStats` runs it from the frames the page received and gets its coverage to the bit and its metrics within the GPU composite's bound. `core/readouts` writes the Frame panel, the mask and time status, the source line, the pipeline bar and its warning, and the clip bar's widths as the page's `showMetrics`, `updatePipe` and `paintClipBar` do, and the app shows them (`AppMeasure`: panel, bars, scopes, a re-measure on a grade change). Found on the way: `toLocaleString` rounds the shortest decimal form (99.9995 to three places is 100.000) where `toFixed` rounds the exact value, so `js_fmt` does the same; Playwright hands a Python 0.0 to the page as -0, so the harnesses pass their inputs as JSON text; and the page's pipeline bar claimed "PQ · Rec.2020" for the view until a first frame was measured, which its own shell.js calls the wrong thing; the markup now says what updatePipe writes |
| 9 | Deliver tab: a master EXR of the frame or the whole sequence as an engine background job with progress and cancel; `render_master` moves out of the CLI into `deliver` so the app and the CLI share it | `ui/server.py` `/api/master/plan` and `/api/master` | the same paths and names as the plan, and the same EXR and sidecar bytes as the Studio for a 3-frame sequence (within the master goldens' bound) | 2 | **done** 24 Sep: `deliver/master` holds the request, `write_master` and the render plan; `master_targets` is `ui/server.py` `master_targets` on 21 cases (`tools/emit_render_plan_golden.py`: the folder, the name, the image or the numbered sequence, and every refusal with the server's message); `publish_master` writes each EXR and sidecar in a `.rudra-render-*` staging folder and links them into place, a copy when a link is refused, never replacing a file. `engine/master_job` renders the targets in order off the UI thread, reports each frame, stops after the frame in hand on a cancel and on the first failure with what was done (`test_master_job`); the app's Master EXR runs it, the button stops it while it renders (`AppDeliver`). The CLI keeps decode and inference only, and `master-check` still passes the Studio's master goldens through the shared code (pixels within 1 half ulp, headers and sidecars equal) |
| 10 | Checkpoint manager and first run: find, verify and switch model packages and the runtime and device (the page's `#ckpt`, `#device`) without a restart; the first-run wizard runs the viewer's HDR card on the display it opens on | `/api/model`, `/api/checkpoints` | switching packages mid-session keeps the session; the wizard reports the display's real peak | 1.5 | **done** 24 Sep: `engine/model_catalog` is the server's discovery for packages (`find_checkpoint` and `loadable_models` on 11 layouts, `tools/emit_catalog_golden.py`): roots in order, a `models.json` root described not guessed (its default, else the first registered SDR2HDR package, the temporal refiner never listed), a training root by shipped then newest export, `_invalid_*` hidden; a folder of packages with no registry opens its first readable package, since a package says what it is. `infer/self_test` is Gate A's golden check moved out of the CLI (`diff` now calls it): the first time a package runs on a backend its goldens run, and one that drifts is refused (`ModelSelfTest`, with a golden value moved). The app: File > Model packages (the list, the runtime and device, Add folder, Use) and Help > Check the display and the model; `use_model` verifies, opens and tests off the UI thread and swaps the backend with the frames, the frame on screen, the grade and undo kept, the old engine's late frames dropped (`AppModels`: 8 tests); a bare start opens the package used last on its backend, else the catalog's pick, and says where it looked when there is none. The first run shows `core/hdr_card` (Gate B's card, now shared with `rudra-viewer-check`) in a viewer of its own on the display it opened on and reports that display's peak and what will clip (`display_report`). The `#device` pill is the page's `info.gpu`: CPU, or the device's name for a GPU; the GPU's own model name needs the CUDA headers and is left for the Windows pass. `tools/export_model.py` puts `models.json` beside the packages |
...
|---|---|---|---|---|
| 1 | Probe and the input contract: find ffmpeg and ffprobe, read ffprobe's JSON, the clock (`timing`: CFR, continuous timestamps) and `input_contract` (HDR, alpha, interlace, rotation, anamorphic, odd sizes, colour tags or explicit overrides) with every refusal | `rudra/video.py` probe, timing, input_contract, has_alpha | the same contract, clock and messages on a set of small clips the emitter makes with ffmpeg | 1| **done** 24 Sep: `platform/process` runs a program with an argument vector, never a shell (`run_process` to completion with stdout and stderr captured; `Process` streams stdout while stderr goes to a log, `kill` on an error path; `CreateProcessW` with the runtime's quoting on Windows, `posix_spawnp` elsewhere; `find_executable` is `shutil.which`), tested with `cmake -E` so it runs on every build machine. `media/video_probe` runs ffprobe with video.py's two argument lists and reads its JSON into `VideoStreamInfo` and per-frame timestamps; `input_contract`, `video_timing`, `has_alpha`, `decoder_filter` and `open_video_source` (convert_video's order: one video stream, contract, clock, alpha, constant frame size) give the Python's contract, clock (exact doubles) and refusal messages. `tools/emit_video_golden.py` makes 14 small clips with ffmpeg (tagged H.264 at 24000/1001 starting at 10 s, untagged, sRGB full range, BT.2020, PQ, HLG, ProRes 4444 with alpha, interlaced, rotated by a display matrix, anamorphic, odd width, a dropped frame, RGB PNG in MOV, two video streams), keeps them once made, and records ffprobe's JSON and the Python's verdict for 23 clip and argument cases (24 and 15 clips since step 3 added colour bars); `test_video` holds the port to them from the recorded JSON on any machine and from a live ffprobe when one is on PATH. One difference: an `avg_frame_rate` of `0/0` raises ZeroDivisionError in the Python and is the zero-rate refusal here |
| 2 | Decode: ffmpeg into 16-bit RGB (and RGBA) on a pipe through `decoder_filter`, frame by frame, a truncated or extra frame refused | convert_video's decode loop | frames equal the Python's decoded arrays, bit for bit | 1| **done** 24 Sep: `media/video_decode`: `decoder_command` is convert_video's ffmpeg command argument for argument (checked against the command the Python itself ran); `VideoDecoder` reads each raw rgb48le or rgba64le frame off the pipe as `read_frame` does, stderr in `decode.log`, and refuses with the Python's words a stream that ends early, a partial frame, extra frames and a failed decoder (with the log's tail); `rgb_from_frame` is `decoded[..., :3].astype(float32) / 65535` as a planar image. The emitter runs convert_video itself on six clips (H.264 limited and full range, untagged with overrides, BT.2020, RGB PNG, ProRes 4444 with alpha) with the predictor stubbed, and records the decoder command and the SHA-256 of every frame `read_frame` returned; the test decodes the clips and holds every frame to those bytes when the ffmpeg is the recording build (its version line is in `decode.json`), and to the frame count and shape otherwise |
| 3 | The video predictor: canonicalise the SDR, convert primaries to Rec.2020, the shadow smoother with its cut detector, residual scale, tiles blended as the Python blends them | `Predictor.predict`, `ShadowSmoother` | HDR frames within the package's tolerance of eager PyTorch on the clips; the same cuts and weights | 1.5| **done** 24 Sep: `core/video_predict` holds the parts around the network in float32 as the tensors are: `canonicalize_sdr` (transfer and range to canonical sRGB code), `srgb_code_to_rec2020`, `area_resize` (aten's adaptive average pool, bins and divisions as it does them), `torch_linspace` (the CPU kernel's fused vector path), `ShadowSmoother` (weight in double, reset at a cut), the tile origins and the video path's own feathers; `engine/video_predictor` runs the frame pass once, smooths its shadow weight, and reconstructs the frame whole or in tiles as the network's fields through the composite with preserve_outside and recovery all, blended and normalised as the Python blends. `tools/emit_video_predict_golden.py` runs `Predictor.predict` on the shipped checkpoint over four sequences (tiled 32/8 with a hard cut half way and retention 0.5, whole-frame BT.2020, RGB with retention 0.9 and cuts off, ProRes 4444 alpha in 24/4 tiles that do not divide the frame), recording the raw 16-bit inputs, the canonical frame and thumbnail it built, the raw and smoothed weights, the cuts and the HDR. `test_video_predict` holds the pure parts to it (linspace and the thumbnail within 1 ulp, the canonical frame within 2e-6, the smoother's weights and cuts exact); `test_video_predict_real` runs the package on LibTorch and ONNX Runtime: every cut and weight the same, HDR within 2.7e-6 of eager PyTorch, tiles included (the package's tolerance is 1e-5 and 3e-4). The emitter added a colour-bars clip to the video goldens so a cut is a real one (24 contract cases, 15 clips, 7 decodes), and `.npy` reads uint16 now |
| 4 | Mastering and the spool: `encode_master` for HDR10, HLG and ProRes (PQ), MaxCLL and MaxFALL per frame, the 16-bit PNG spool (alpha carried), disk space checks | `delivery/profiles.py`, the spool writer | the same code values and the same PNG bytes | 1 | **done** 24 Sep: `deliver/video_master`: `master_video_frame` is the loop body between the predictor and the spool (the finite checks with the Python's messages, `encode_master` for the format, MaxCLL and the frame average as float32 `maxrgb.max()` and `maxrgb.mean()`, the latter numpy's pairwise float32 sum now in `core/numeric`, then `(clip(code) * 65535 + 0.5)` to uint16, BGR, the decoded alpha appended); `light_ceilings` gives the encoder's MaxCLL and MaxFALL; `spool_space_low` is the disk check. `media/png16` writes and reads the spool with OpenCV as `cv2.imwrite` does. `tools/emit_video_master_golden.py` runs convert_video itself on two clips with the step 3 HDR frames standing in for the predictor and stops it at `encode_command`, keeping the spool it wrote and the two ceilings, for HDR10 at 1000 and at 4000 nits with a 1000-nit knee, HLG at 1000 and 600, and ProRes 4444 with alpha (30 frames). `test_video_master`: HLG codes identical, every frame's MaxCLL and average identical, the ceilings identical, the float32 mean identical to numpy's on 12 sizes up to 1080p; PQ codes within one of 65535 on about 1.6 % of samples, because numpy's float32 power is SIMD code (SVML on AVX-512) that is not correctly rounded, so the Python's own PQ codes depend on the CPU, while libm's `powf` here is. PNG bytes are compared when the codes are identical and OpenCV is the recording build (this container's Python has 4.13, the C++ 4.6) |
| 5 | Encode and publish: the ffmpeg command, argument for argument, for all five presets; staged beside the output and renamed into place; nothing replaced | `encode_command` | the same argv; outputs decode | 1| **done** 24 Sep: `deliver/video_encode`: `encode_command` builds the ffmpeg command as the Python does (the x265 parameters with the master-display string rounded half to even and the MaxCLL pair, HLG's own parameters, ProRes profile, tag and alpha bits, the audio map and codec, the start offset and duration as Python's `repr`, `+faststart+write_colr` for MP4 and MOV in any case); `StagingDir` is the temporary folder beside the output, removed with its owner; `publish_video` refuses when either target appeared and otherwise renames the master and its sidecar into place. The step 4 emitter now also records the command convert_video itself built on ten runs (HDR10, HLG, ProRes 422, 422 HQ and 4444 with alpha, AAC with CRF 18 and preset slow, no audio into MKV, HLG into `.MOV`, start 10 s and 0 s); `test_video_encode` matches each argument, encodes the Python run's own spool with the native command for HDR10, HLG and ProRes 4444 when ffmpeg is on PATH (the right codec, transfer and frame count), and checks publishing never replaces a file or a sidecar. `VideoClock` moved to `core/video_clock.hpp` so deliver reads it |
| 6 | QC: `quality_check` (codec, pixel format, tags, frame count, every timestamp, HDR10 SEI and its values, HLG without static metadata, ProRes tag, duration, audio count, rate, channels, offsets, a complete decode) and `check_alpha` | `quality_check`, `check_alpha` | the same pass or the same errors on good and broken files | 1| **done** 24 Sep: `deliver/video_qc`: `evaluate_video_qc` makes every check of `quality_check` on ffprobe's JSON (read in its own key order, so the record keeps it) with the Python's words and order, `qc_verdict` gives "Export QC failed: ..." or the record the sidecar carries, `alpha_verdict` is check_alpha's; `media/video_alpha` extracts the master's alpha on a pipe and compares every pixel with the spool's fourth channel; `engine/video_qc` probes the master, checks it, decodes it completely, then gives the verdict, as convert_video does. `platform/process` took `require_executable` and `run_tool`, and `core/video_clock` took `Fraction`, so deliver reads them. `tools/emit_video_qc_golden.py` has convert_video make five masters end to end (HDR10 with audio copied and with AAC, HLG, ProRes 422, ProRes 4444 with alpha; a clip with audio added to the video goldens), derives five broken ones (a transfer tag changed in the bitstream, a frame dropped, the audio removed, re-encoded without the HDR10 SEI, a wrong ProRes tag) and records 14 verdicts, adding the wrong mode, HDR10 checked as HLG, light metadata that disagrees and ProRes 422 checked as 4444. `test_video_qc`: every verdict and message the same from the recorded probes and live, each passing record byte for byte as `json.dumps` writes it (live when the ffmpeg is the recording build), and the alpha record the same as the sidecar's |
| 7 | `rudra-native video`: convert_video end to end, the sidecar report key for key | `convert_video` | on three clips (HDR10 with audio, HLG, ProRes 4444 with alpha) the published file passes QC and decodes to the Python's frames; the report's keys and values match | 1| **done** 24 Sep: `video/convert` is convert_video: the argument and output checks in the Python's order and words (suffix, ProRes needs MOV, nothing overwritten, the encoders and zscale in this ffmpeg, smoothing, tiles, CRF and mastering, then the peak and knee), the probe and contract, the disk check, the decode, predictor, mastering and spool per frame with the progress the queue reads and the lines the Python prints, the encoder, QC and the alpha check on the staged master, the HLG reference, and the report written as `json.dumps(report, indent=2)` and published beside the master; a cancel flag is checked once a frame for the app. It lives in a new `video/` layer between infer, media and deliver and `engine/`, so `rudra-native video <package> <input> --output ...` (all of video.py's options, plus `--runtime` and `--device`) runs it without Qt; the predictor and QC composer moved there from engine. `tools/emit_video_convert_golden.py` runs convert_video on the checkpoint on three clips (HDR10 with audio copied, HLG at 1200 nits smoothed at 0.5 in 32/8 tiles on colour bars, ProRes 4444 with alpha and no audio) and keeps the masters and reports. `test_video_convert_real` on LibTorch and ONNX Runtime: every report key in the same order with the same value (the paths, spool and staging folders and elapsed time aside; per-frame weights and light within the package's tolerance, a few of them not bit-equal), QC passed; the HDR10 and HLG masters byte-identical to the Python's on both runtimes, the ProRes 4444 master within four 10-bit steps anywhere and 0.35 of a 16-bit code on average when decoded (its PQ codes differ by one in 65535 where numpy's power does, step 4, and ProRes is lossy); every refusal before any work, with the Python's words |
| 8 | Frames to video: `encode_sequence` (a finished HDR sequence to HDR10, HLG or ProRes) | `rudra/delivery/video.py` | the same argv, QC and tags | 1| **done** 24 Sep: `deliver/sequence_encode` ports encode_sequence: the four targets, `encode_sequence_frame` (primaries to Rec.2020 through float32 as the Python goes, the shoulder into the peak, PQ, HLG through the inverse OOTF, 16-bit rounding), the command with the swscale BT.2020 matrix, `+write_colr` and `prores_metadata` when this ffmpeg has them (`ffmpeg_supports`, cached), the x265 VUI and HDR10 parameters; frames go down ffmpeg's stdin one at a time (`platform/process` gained a stdin-fed `Process::start_writer`, SIGPIPE ignored as a broken pipe is in Python), and the tags are read back where each format keeps them (`colour_tags` through ffprobe with its old-build retry, `prores_frame_tags` from the frame header, `container_colr`), refusing an untagged file with the Python's words. `rudra-native deliver <frames> --output <stem> ...` is `rudra deliver`: .exr and .npy frames in name order, MaxCLL and MaxFALL measured after the shoulder, the same JSON report. `tools/emit_sequence_encode_golden.py` records _encode_frame's codes for every target from Rec.709, Rec.2020 and P3 with and without the shoulder, and `rudra deliver` on three 128 x 72 frames for each target (the command it ran, its report, the tags and colr atom of the file). `test_sequence_encode`: codes within one of 65535 (numpy's SIMD power and log are not correctly rounded, libm's are), every command argument, MaxCLL and MaxFALL, the tags of live encodes, and the CLI's report byte for byte. Found on the way: x265 3.5 crashes (exit 139) at 48 x 32 with preset slow, in the Python as here, so the goldens use 128 x 72; and two native tests iterated a temporary's JSON (a dangling range-for) and silently ran no cases, now fixed |
| 9 | The queue runs video jobs: `run_queue` with its locks, digests, retry and resume | `rudra/batch.py` | a three-clip queue killed mid-clip resumes to the Python's final state; each side resumes the other's queue | 1| **done** 24 Sep: `video/queue_runner` makes convert_video the queue's runner: a job names its checkpoint as the Python does (the `.pt`, whose digest is its identity), and the package that runs it is the one exported from that checkpoint, by the manifest's source SHA-256 (`--package`, or `RUDRA_PACKAGE_ROOTS`, `dist/models` and the queue's folder); the backend is opened once per checkpoint and device; progress goes into the state as the Python's dict. A stop (`ErrorCode::Cancelled`, from Ctrl+C in the CLI or the app's flag, checked once a frame) is the Python's KeyboardInterrupt: the job is left `interrupted` with its words and the queue ends. `rudra-native batch run <queue> [--retry-failed] [--package] [--runtime] [--device]` and `batch status`. `tools/emit_video_queue_golden.py` runs a three-clip queue (HDR10 with audio, HLG smoothed, ProRes 4444 with alpha) through `run_queue` straight and stopped at the second clip, keeping the stopped project. `test_video_queue_real`: the Python's stopped queue resumes here (job 1 verified and skipped, its master untouched) to the Python's final state key for key, artifacts held to the files; a native run stopped the same way leaves the Python's interrupted state and resumes to the final one; a job without its package fails and the queue moves on. By hand: `rudra-native batch run` stopped with Ctrl+C at the second clip, then `rudra batch run` in Python on the same queue: job 1 verified, skipped, the rest complete, exit 0 |
| 10 | ffmpeg on the machine: capability probe (encoders, zscale) and a 16-frame HDR10 self-test per ffmpeg binary, cached by its hash | the probe in convert_video | a missing encoder or a broken build is reported before any work | 0.5| **done** 24 Sep: `video/ffmpeg_check`: `probe_ffmpeg` finds ffmpeg and ffprobe, reads the encoder and filter tables for libx265, prores_ks, zscale and alphaextract (whole names, so `libx265rgb` is not `libx265`), asks for `write_colr` and `prores_metadata`, and hashes both binaries; `missing()` says in words what a delivery would lack. `ffmpeg_self_test` runs the real pipeline on 16 frames (a tagged SDR clip every build can make, lavfi into FFV1, decoded through zscale, mastered, spooled, encoded to HDR10 with its SEI, QC'd) and keeps a pass in the cache (`default_cache_dir`: `RUDRA_CACHE_DIR`, `%LOCALAPPDATA%\RUDRA`, `~/Library/Caches/RUDRA` or `$XDG_CACHE_HOME/rudra`) keyed by both binaries' SHA-256 and the version line (a packaged ffmpeg is a small binary over shared libraries); a build without libx265 or zscale fails with convert_video's own words before anything runs. `rudra-native ffmpeg-check [--ffmpeg] [--ffprobe] [--force] [--no-self-test]` prints the report and exits 1 when something is missing. `test_ffmpeg_check`: the PATH build's capabilities, a stand-in ffmpeg without libx265 and zscale refused before any work, the self-test passing in 0.7 s and cached until the build changes. The queue keeps the Python's behaviour (each job reports a missing encoder itself); the app runs this check before its first export (step 11) |
| 11 | The app: a movie opens as a shot (frames decoded on demand), the export sheet's HDR10, HLG and ProRes tiles go live as queue jobs, a queue window with progress and resume | the Pro export board | a movie scrubs, grades and exports from the window | 1.5| **done** 24 Sep: a movie opens as a shot as the Studio opens it (`media/sequence`: ui/sequence.py's video half, counted by packets with ffprobe, the rate as a float, each frame extracted on first use with `-accurate_seek -ss` into the Studio's cache folder, `sha1` of the resolved path, written then renamed; `ensure_frame_file` lets the engine and the master decode a frame that is not there yet; `platform/hash` gained SHA-1). In the window: Open and a drop take movies, the shot's title, frame names (`<stem>_000001`) and rate are the movie's. The export sheet's HDR10, HLG and ProRes 422 HQ tiles are live for a movie: the file is named from the Deliver tab, this ffmpeg is asked for libx265 or prores_ks and zscale first, the export is written as a queue file in batch.py's format beside the master (`<name>.<format>.queue.json`, the package's manifest as its checkpoint) and run on a worker (`app/video_queues`, one queue at a time, on the backend in use); nothing existing is replaced. The queue window (`app/queue_window`) shows each export's state as the runner writes it (waiting, reconstructing frame N of M, encoding, checking, complete, failed with its reason, stopped), with Stop, Resume and Remove, and the list is kept between runs. `RUDRA --workflow-check ... --movie CLIP` adds the movie to the scripted workflow: opened, every frame scrubbed, HDR10 exported through the queue and its QC read back. Tests: `Sequence.AVideoOpensAndReadsLikeThePython` (the Studio's describe() and its extracted PNGs byte for byte), `AppVideo` (a dropped movie opens, scrubs and names its frames; the tiles, the queue file, the window's words, refusals and the list kept). On Linux with ONNX Runtime the workflow's movie step passes and its HDR10 master is byte-identical to the one `rudra video` makes from the same clip |
| 12 | Review: the scripted and by-hand pass on Windows (an ffmpeg with libx265 and zscale beside the app), `STATUS.md` | | a clip with audio in, an HDR10 master out that passes QC, from the app and from the queue | 0.5| **scripted, Linux passes** 24 Sep; the Windows run is `scripts/NATIVE_PHASE4_EXIT.ps1 -Clip <movie>`: the Phase 3 build with the native tests, `rudra-native ffmpeg-check` on the machine's ffmpeg, the video pipeline's tests against the goldens, `rudra-native video` to HDR10 with audio and to ProRes 422 HQ (QC read back from each report), a three-job queue stopped hard as its second job starts and run again to completion, and `RUDRA --workflow-check --movie` on a PATH of the app, ffmpeg and Windows only (the Phase 3 workflow on the clip's first 48 frames, then the movie scrubbed and exported through the app's queue), with the by-hand checklist. On Linux (ONNX Runtime CPU) every part passes; the app's movie export is byte-identical to `rudra video`'s |
