...
rudra deliver out/ --output delivery/shot --target prores4444 --fps 24     # ProRes / HDR10 / HLG
...
`deliver` writes one file a timeline will take: `prores4444` to finish in,
`prores422hq` to review, `hdr10` to deliver, `hlg` for broadcast paths that
will not take PQ. MaxCLL and MaxFALL are measured from your frames rather than
typed in, and the file is read back before you get it, to confirm the Rec.2020
and PQ tags actually landed. That check matters more than it sounds. An
untagged file plays everywhere, looks washed out, and no player tells you why.

Each format is checked where it actually keeps that information. ProRes writes
its colour primaries, transfer and matrix into every frame header, which is
what Resolve and Final Cut read. HEVC writes them into the bitstream, which is
what ffprobe reads. If something looks wrong, run
`python training/diagnose_delivery_tags.py`: it prints what your ffmpeg build
does for each of the four targets, in both places, so you can see whether a
file is genuinely untagged or only looks that way to one tool.
